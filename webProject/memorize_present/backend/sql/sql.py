...
def select(name, fromName, filterSql):
    sql = 'SELECT {} FROM {} WHERE Is_del=0 {}'.format(
        str(name), str(fromName), str(filterSql))
...
def update(fromName, setSql, filterSql):
    sql = 'UPDATE {} SET {} WHERE 1=1 {}'.format(
        str(fromName), str(setSql), str(filterSql))
    data = cursor.execute(sql)
    conn.commit()
    return data
...
class Sql():
    ...
    def uploadFile(parm):
        ...
        if bool(parm):
            if ('fileName' in parm):
                ...
                filter = "(File_name, Upload_time, Upload_user) VALUE ('{}', '{}', '{}')".format(
                    str(parm['fileName']), str(timeStr), str(parm['userName']))
    ...
    def setIntervalConnSql():
        res = select('*', 'User_message', 'AND 1 != 1')
